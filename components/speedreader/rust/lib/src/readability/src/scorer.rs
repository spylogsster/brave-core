...
pub fn clean<S: ::std::hash::BuildHasher>(
    ...
    let useless = match handle.data() {
        ...
        Element(ref data) => {
            let delete = match data.name.local {
                ...
                local_name!("h1") | local_name!("h2") => {
                    ...
                    if data.name.local == local_name!("h1") {
                        ...
                        // If <h2> has class attribute with a negative pattern (ad, hidden, etc.)
                        // remove it.
            ...
            if !delete {
                // Delete style, align, and other elements that will conflict with the
                // Speedreader stylesheet.
